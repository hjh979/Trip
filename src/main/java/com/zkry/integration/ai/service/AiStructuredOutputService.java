...
import com.zkry.integration.ai.support.AiJsonRepairer;
...
public class AiStructuredOutputService {
    ...
    private <T> Optional<T> callAndConvert(
        ...
        try {
            ...
            String repaired = AiJsonRepairer.repairUnescapedQuotes(response.get());
            if (!repaired.equals(response.get())) {
                try {
                    T converted = converter.convert(repaired);
                    log.info("[AI-STRUCTURED] 非法字符串引号已自动修复 agent={} threadId={} responseLength={} elapsedMs={}",
                        agent.id(), threadId, response.get().length(), System.currentTimeMillis() - startedAt);
                    return Optional.ofNullable(converted);
                } catch (Exception repairException) {
                    log.warn("[AI-STRUCTURED] 自动修复后仍无法解析 agent={} threadId={} reason={}",
                        agent.id(), threadId, repairException.getMessage());
                }
            }
